...
const PacketStocks = () => {
  ...
  const [filteredWarehouse_Stock_List, setFilteredWarehouse_Stock_List] = useState([]);
  ...
  const handleDeleteBrand = async (id) => {
    ...
    const previousFilteredWarehouse_Stock_List = [...filteredWarehouse_Stock_List];

    const updatedList = Warehouse_Stock_List.filter((p) => p.warehouse_id !== id);
        ...
        <table className="min-w-full bg-white table-auto border border-gray-300 rounded-lg shadow-sm">
          ...
          <tbody>
            ...
            {filteredWarehouse_Stock_List.length > 0 ? (
              ...
              >
                       <td className="py-3 px-4 text-center">{p.warehouse_unique_id}</td>
                <td className="py-3 px-4 text-center">{p.warehouse_name}</td>
                <td className="py-3 px-4 text-center">{p.warehouse_pin_code}</td>
               <td
  className="py-3 px-4 text-center text-blue-500 cursor-pointer"
 onClick={() =>
  window.open(
    `/admin/${localStorage.getItem('Merchanttoken')}/packetStock/view/${p.warehouse_unique_id}`,
    '_blank'
  )
}
