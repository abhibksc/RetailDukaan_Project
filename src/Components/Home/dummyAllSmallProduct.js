...
import { addCart, cartReducer, decreaseCart, importCartItems, increaseCart } from "../../../ReduxStore/Slices/cartSlice";
import { DecreaseItemQuantityToCart, IncreaseItemQuantityToCart, StoreItemToCart } from "../../../CrudOperations/PostOperation";
import LoadingModal from "../../../LoadingModal";
import { getAllCartItems, getAllItem, getFeaturedBannerUptoOfferItems, getMainBannerUptoOfferItems } from "../../../CrudOperations/GetOperation";
import ItemsSidebar from "../../../Product/ItemsSidebar";
import Sub_SubCategory_Carausal from "./Sub_SubCategory_Carausal";
  ...
  const handleSortChange = (e) => {
    ...
    switch (sortBy) {
      case "PriceLowToHigh":
        sortedProducts.sort(
          (a, b) =>
            (a.Varients[0].loose_product_price ||
              a.Varients[0].packet_varient_selling_price) -
            (b.Varients[0].loose_product_price ||
              b.Varients[0].packet_varient_selling_price)
        );
        break;
      case "PriceHighToLow":
        sortedProducts.sort(
      ...
      case "Discount":
        sortedProducts.sort(
          (a, b) =>
            (b.Varients[0].loose_product_discount ||
              b.Varients[0].packet_varient_discount_percentage) -
            (a.Varients[0].loose_product_discount ||
              a.Varients[0].packet_varient_discount_percentage)
        );
        break;
      default:
        break;
...
{locationState &&      <div className="">
      <Sub_SubCategory_Carausal locationStateData={locationState}/>
      </div>}
      ...
      <div className=" flex flex-col gap-4 border  w-full">
        <div className="bg-white px-4 py-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols- xl:grid-cols-4 gap-4 p-2">
            {!emptyItem &&
              filteredProducts.map((product) => {

                const discountedPrice = (
                  product.sellingPrice -
                  (product.sellingPrice * product.discount_percentage_in_mrp) /
                    100
                ).toFixed(2);
