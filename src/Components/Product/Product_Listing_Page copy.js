...
function ProductListingPage() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      ...
      // Choose what to fetch based on what params are available
      try {
        if (subSubCategory_id && pincode) {
        ...
        } else if (subCategory_id && pincode) {
          const response = await getAllItem({
            id: subCategory_id,
            type: "SubCategory",
            pincode,
    ...
    fetchItems();
  }, [group_id, category_id, subCategory_id, subSubCategory_id]);
  ...
  //     if (location.state) {
  //       const { id, type, bannerId, ImageId, groupId } = location.state || {};
  ...
  //           response?.data?.MainBanner?.length > 0
  //         ) {
  ...
  const handleSortChange = (e) => {
    ...
    switch (sortBy) {
      ...
      case "Discount":
        sortedProducts.sort(
          (a, b) => b.discount_percentage_in_mrp - a.discount_percentage_in_mrp
        );
        break;
            ...
            {!emptyItem &&
              filteredProducts.map((product) => {
                const discountedPrice = (
                  product.mrp -
                  (product.mrp * product.discount_percentage_in_mrp) / 100
                ).toFixed(2);
                ...
                return (
                  ...
                  >
                    <Link
                      //  path="/:groupName/:id/:categoryName/:id/:subCategoryName/:id/subSubCategoryName/:id/:item/:VariantId"
                      to={`/product/${product.category.groupName}/${product.category.groupId}/${product.category.categoryName}/${product.category.categoryId}/${product.category.subCategoryName}/${product.category.subCategoryId}/${product.category.subSubCategoryName}/${product.category.subSubCategoryId}/${product.Modal_Item_id}/${product.variant_id}/${product.Varient_type}/${product.purchase_item_id}/${product.stock_id}/${area_pin}`}
                    ...
                    >
                      {/* Product Image */}
                      <div className="relative overflow-hidden rounded-md text-[10px]">
                        <img
                      ...
                      <div className="flex items-center mt-1">
                        ...
                        {product.discount_percentage_in_mrp > 0 && (
                        ...
                        {product.discount_percentage_in_mrp > 0 && (
                          <div className="text-green-600 ml-2">
                            {Math.round(product.discount_percentage_in_mrp)}%
                            Off
                          </div>
                        )}
                      </div>
                    ...
                    {product.is_Deliverable && product.is_available_in_stock ? (
                      <div className="flex mt-2">
                        <button
                          className={`flex-1  border rounded px-1 py-1 text-gray-700 hover:bg-gray-100 ${
                        ...
                        {isInCart(product.sku_id) ? (
                          ...
                          <button
                            className="flex-1 bg-blue-500 text-white  rounded p-1 py-2 ml-2 hover:bg-blue-600"
                            onClick={() =>
                      ...
                      {`${ele["quantity/pct"]}`}
                        ...
                        <div className="flex items-center space-x-4 mt-2">
                          ...
                          <div className="flex items-center text-lg font-semibold text-gray-700">
                            ...
                            <span>
                              {ele.mrp - ele.discount_percentage_in_mrp / 100 ||
                                "N/A"}
                            </span>
                          </div>
